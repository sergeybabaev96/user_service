...
class UserFilterByCityPatternTest {
    ...
    private static final UserFilterDto PRESET1 = UserFilterDto.builder().cityPattern(PATTERN1).build();
    private static final UserFilterDto PRESET2 = UserFilterDto.builder().cityPattern(PATTERN2).build();
    ...
    @BeforeAll
    static void init() {
        filter = new UserFilterByCityPattern();
    }

    @BeforeEach
    void setUp() {
        users = Stream.of(
                User.builder().city(CONTENT1).build(),
                User.builder().city(CONTENT2).build(),
                User.builder().city(CONTENT3).build()
        );
    }
