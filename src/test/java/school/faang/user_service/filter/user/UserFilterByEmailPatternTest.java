...
class UserFilterByEmailPatternTest {
    ...
    private static final UserFilterDto PRESET1 = UserFilterDto.builder().emailPattern(PATTERN1).build();
    private static final UserFilterDto PRESET2 = UserFilterDto.builder().emailPattern(PATTERN2).build();
    ...
    @BeforeAll
    static void init() {
        filter = new UserFilterByEmailPattern();
    }

    @BeforeEach
    void setUp() {
        users = Stream.of(
                User.builder().email(CONTENT1).build(),
                User.builder().email(CONTENT2).build(),
                User.builder().email(CONTENT3).build()
        );
    }
