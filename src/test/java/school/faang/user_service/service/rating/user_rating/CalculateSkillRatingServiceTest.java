...
class CalculateSkillRatingServiceTest {
    @Mock
    ...
    @InjectMocks
    CalculateSkillRatingService calculateSkillRatingService;
    ...
    void calculate() {
        ...
        when(ratingTypeService.findByName(RatingType.SKILL_RATING)).thenReturn(ratingType);
        ...
        List<UserRating> actualRatings = calculateSkillRatingService.calculate(userIds);
        ...
        assertEquals(RatingType.SKILL_RATING, actualRatings.get(0).getType().getName());
        assertEquals(RatingType.SKILL_RATING, actualRatings.get(1).getType().getName());

        verify(ratingTypeService, times(1)).findByName(RatingType.SKILL_RATING);
    ...
    void calculateUserIdsIsEmpty() {
        assertEquals(List.of(), calculateSkillRatingService.calculate(List.of()));
    }
}